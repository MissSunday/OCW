...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.row == 0) {
        ...
        dispatch_async(dispatch_queue_create("tr", DISPATCH_QUEUE_CONCURRENT), ^{
            for (int i = 0; i < 30; i ++) {
                [self write];
            }
        });
        dispatch_async(dispatch_queue_create("tg", DISPATCH_QUEUE_CONCURRENT), ^{
            for (int i = 0; i < 30; i ++) {
                [self read];
            }
        });
...
-(void)read{
    ...
    NSLog(@"读 - %ld  %@",self.ticketNumber,[NSThread currentThread]);
...
-(void)write{
    //[self.lock lock];
    dispatch_semaphore_wait(self.semap, DISPATCH_TIME_FOREVER);
    NSInteger a = self.ticketNumber;
    NSLog(@"读 - %ld  %@",self.ticketNumber,[NSThread currentThread]);
    a--;
    self.ticketNumber = a;
    NSLog(@"写 - %ld  %@",_ticketNumber,[NSThread currentThread]);
    dispatch_semaphore_signal(self.semap);
    //[self.lock unlock];
}
