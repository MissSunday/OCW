...
// !!!: 额外添加属性
@property(nonatomic,assign)BOOL setSelectedIndexNeedReturn; //主动设置选中索引时是否走回调代理,默认YES 这里为了的是  桥接的scrollview不能判断当前的索引  比如 一个按钮对应多个分页 图片浏览分区
...
// 跟踪器颜色 (wxr额外添加,我也不明白这么叼的大佬写的轮子,怎么非得用style控制这个颜色呢,要是想自定义呢,没有预留字段,所有加了个)
...
@property (nonatomic, assign) CGFloat trackerY; // 更变y值
